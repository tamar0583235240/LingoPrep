...
export async function analyzeAudio(filePath: string): Promise<IAiAnalysis> {
  ...
  try {
    ...
    // יצירת חיבור לשירות Gemini באמצעות ה-API Key
    const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
    console.log('אובייקט GoogleGenerativeAI נוצר.');
    // הגדרת המודל ל-gemini-2.5-pro עם הגדרות יצירה ספציפיות
    // חשוב: הגדרת responseMimeType מבטיחה שהמודל יחזיר JSON תקין.
    // הגדלת maxOutputTokens מאפשרת תגובות ארוכות יותר.
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-pro", // המודל המבוקש לניתוח אודיו ישיר
      generationConfig: {
        temperature: 0.9,
        topK: 40,
        topP: 0.95,
        maxOutputTokens: 4096, // הגדלנו את אורך התשובה המקסימלי
        responseMimeType: "application/json" // הנחיה מפורשת למודל להחזיר JSON
      }
    });
    ...
    // בדיקה וטיפול בתגובה מהמודל
    if (!geminiResult || !geminiResult.response) {
      console.error('שגיאה: לא התקבלה תגובה חוקית מהמודל של Gemini.');
      throw new Error('לא התקבלה תגובה חוקית מ-Gemini');
    }
    const responseText = geminiResult.response.text();
