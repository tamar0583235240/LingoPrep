...
export const validateCode = async (req: Request, res: Response) => {
  ...
  if (!validCode) {
    return res.status(200).json({
      valid: false,
      message: "לא נמצאה בקשה לקבלת קוד למייל זה. אנא נסה שנית.",
    });
  }
...
// בקשת איפוס סיסמה - יצירת טוקן ושליחה במייל
