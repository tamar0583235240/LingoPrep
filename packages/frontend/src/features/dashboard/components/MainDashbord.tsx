...
import { store, RootState } from "../../../shared/store/store";
...
const MainDashboard: React.FC = () => {
  ...
  const userName = useSelector(
    (state: RootState) => state.auth?.user?.firstName ?? "משתמש ללא שם"
  );
  ...
  // סטטוס התקדמות מה-Zustand (אם אתה ממשיך להשתמש בו)
  const { answered, total } = useUserStore();
  const isComplete = answered === total && total > 0;

  // סגירת תעודה כאשר לוחצים מחוץ לה
      ...
      {showCertificate && (
        ...
        >
          <div ref={certificateRef}>
            <Certificate fullName={`${userName} ${userLastName}`} />
          </div>
        </div>
      )}
